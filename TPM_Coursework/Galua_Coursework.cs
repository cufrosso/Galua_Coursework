...
namespace Galua
{
                ...
                if (q > 1)
                {
                    if (m > 0)
                    {
                        ...
                        if (is_prime(q))
                        {
                            ...
                            {
                                // Находим Coef по значениям q и m
                                string coef = context.I_Poly
                                    .Where(ip => ip.q == q && ip.m == m)
                                    .Select(ip => ip.Coef)
                                    .FirstOrDefault();

                                string[] coefArray = coef.Split(',');
                                coeff = Array.ConvertAll(coefArray, int.Parse);
                            }
                            for (int i = 0; i <= m; i++)
                            {
                                ((ComboBox)g_boxes[i]).SelectedIndex = coeff[i];
                            }
                        }
                        else MessageBox.Show("Введеный вами q не является простым");
                    }
    ...
    public class ApplicationContext : DbContext
    {
        ...
        public DbSet<I_Poly> I_Poly { get; set; }
